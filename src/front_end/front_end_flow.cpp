...
FrontEndFlow::FrontEndFlow(ros::NodeHandle& nh) {
    ...
    imu_sub_ptr_ = std::shared_ptr<IMUSubscriber>(new IMUSubscriber (nh, "/kitti/oxts/imu", 1000000));
    ...
    gnss_pub_ptr_ = std::shared_ptr<OdometryPublisher>(new OdometryPublisher(nh, "gnss", "map", "lidar", 100));
...
bool FrontEndFlow::UpdateGNSSOdometry() {
    ...
    gnss_odometry_(0,3) = current_gnss_data_.local_x;
    gnss_odometry_(1,3) = current_gnss_data_.local_y;
    gnss_odometry_(2,3) = current_gnss_data_.local_z;
    gnss_odometry_.block<3,3>(0,0) = current_imu_data_.GetRotateMat();
...
bool FrontEndFlow::SaveTrajectory() {
    ...
    if (!is_file_created) {
        if (!FileManager::CreateDirectory("/home/jingwan/lidar_ws/src/lidar_test/slam_data/trajectory"))
            return false;
        if (!FileManager::CreateFile(ground_truth, "/home/jingwan/lidar_ws/src/lidar_test/slam_data/trajectory/ground_truth.txt"))
            return false;
        if (!FileManager::CreateFile(laser_odom, "/home/jingwan/lidar_ws/src/lidar_test/slam_data/trajectory/laser_odom.txt"))
            return false;
